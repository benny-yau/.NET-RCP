...
namespace FlashDevelop.Managers
{
    class SessionManager
    {
                    ...
                    if (!te.Handled)
                    {
                        for (Int32 i = 0; i < session.Files.Count; i++)
                        {
                            ...
                            if (!(String.IsNullOrEmpty(fileToOpen)))
                                Globals.MainForm.BrowseWebsite(fileToOpen);
                        }
        ...
        public static Session GetCurrentSession()
        {
            ...
            for (Int32 i = 0; i < documents.Length; i++)
            {
                ...
                foreach (Control ctrl in document.Controls)
                {
                    if (ctrl is Browser)
                    {
                        Uri browserUri = (ctrl as Browser).WebBrowser.Url;
                        if (browserUri != null && !(String.IsNullOrEmpty(browserUri.ToString())))
                ...
                session.Files.Add(url);
                AddDocumentDock(document, session, url);
            }
            return session;
        }
        ...
        /// </summary>
        public static void AddDocumentDock(ITabbedDocument document, Session session, String url)
        {
            try
            {
                ...
                if (nestIndex > -1)
                {
                    NestedDock dock = new NestedDock(url, nestIndex, paneIndex, align, prop); //document.FileName
