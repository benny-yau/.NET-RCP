...
namespace PluginCore.Managers
{
    public class ErrorManager
    {
        ...
        public static DialogResult ShowQuestion(String question)
        {
            if ((PluginBase.MainForm as Form).InvokeRequired)
            {
                AddToLog("Unsafe ShowQuestion: " + question, null);
                return DialogResult.Cancel;
            }
            String title = TextHelper.GetString("FlashDevelop.Title.ConfirmDialog");
            return MessageBox.Show(PluginBase.MainForm, question, " " + title, MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
        }
